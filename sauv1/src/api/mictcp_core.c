...
int initialize_components(start_mode mode)
{
    ...
    else
    {
        if(initialized != -1)
        {
            ...
            remote_addr.sin_port = htons(API_Port);
...
int IP_recv(mic_tcp_payload* pk,mic_tcp_sock_addr* addr, unsigned long delay)
{
    ...
    {   
        int recv_size = recvfrom(sys_socket, pk->data, pk->size, 0, (struct sockaddr *)&remote_addr, &remote_size);
        pk->size = recv_size;
        return recv_size;
    }
...
int partial_send(mic_tcp_payload buff)
{
    ...
    if(range == 0)
    {
        jump = 100.0 /((float) loss_rate);
        range += jump;
    }
    
    printf("\n - %f, %d", range, count);
    
    if(floor(range) == count)
    {
        reverse = -reverse;
        printf(" -- Delete");
        jump = 100.0 /((float) loss_rate);
...
int app_buffer_get(mic_tcp_payload app_buff)
{
    ...
    if(app_buffer_count > 0)    
    {
        ...
        memcpy(app_buff.data, tmp.data, max_size(tmp.size, app_buff.size));
